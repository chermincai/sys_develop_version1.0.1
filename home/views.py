...
def text_test(request):
    ...
    if request.method == 'POST':
        ...
        model_input = "我将输入一些内容，你来判别它们是否是不良的，如果是，请分析它的类别同时给出判断的置信度，以百分数的形式展示，不要重复需判断文本的内容，。你对输入文本的分类将在涉政敏感类，暴力恐怖类，宗教迷信类，色情类，仇恨谩骂类，诈骗类，赌博类，涉毒类，不良广告类，自残自杀类，校园霸凌类，负面社会民生类这十二类里产生。需判断的文本的内容是：" + user_input
